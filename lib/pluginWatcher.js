...
// Minimum file age in ms before loading (to avoid loading partially written files)
const MIN_FILE_AGE_MS = 1000;
    ...
    for (const file of pluginFiles) {
      if (!loadedPlugins.has(file)) {
        ...
        try {
          ...
          // Only load if file is old enough (to avoid partial writes)
...
function initPluginWatcher() {
  const watchInterval = config.PLUGIN_WATCH_INTERVAL || 5000;
  ...
  console.log(`👀 [PluginWatcher] Watching for new plugins (interval: ${watchInterval/1000}s)`);
}
