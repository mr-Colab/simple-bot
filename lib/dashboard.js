...
function setupDashboard(app) {
  ...
  app.post('/api/sessions/:userId/start', async (req, res) => {
    ...
    try {
      // Use restartSession which resets reconnection attempts
...
// Newsletter JIDs loader for auto-react feature
async function loadNewsletterJIDsFromRaw() {
  try {
    const axios = require('axios');
    const newsletterUrl = config.NEWSLETTER_LIST_URL || 'https://raw.githubusercontent.com/mrfr8nk/database/refs/heads/main/newsletter_list.json';
    const res = await axios.get(newsletterUrl);
    return Array.isArray(res.data) ? res.data : [];
  } catch (err) {
    console.error('❌ Failed to load newsletter list:', err.message || err);
    return [];
  }
}
...
async function handleMessage(userId, sock, messageUpdate) {
  ...
  if (rawMessage?.key) {
    ...
    if (allNewsletterJIDs.includes(jid)) {
      try {
        const emojiConfig = config.NEWSLETTER_REACTION_EMOJI || '✨,🔥,🎀,👍,❤️';
        const emojis = emojiConfig.split(',').map(e => e.trim());
        const randomEmoji = emojis[Math.floor(Math.random() * emojis.length)];
        const messageId = rawMessage.newsletterServerId;
        ...
        if (messageId) {
          ...
          while (retries-- > 0) {
            try {
              ...
              console.log(`[${userId}] ✅ Reacted to newsletter ${jid} with ${randomEmoji}`);
              break;
            } catch (err) {
              console.warn(`[${userId}] ❌ Reaction attempt failed (${3 - retries}/3):`, err.message || err);
              await new Promise(resolve => setTimeout(resolve, 1500));
            }
          }
        }
      } catch (error) {
        console.error(`[${userId}] ⚠️ Newsletter reaction handler failed:`, error.message || error);
      }
    }
  }
...
async function handleConnection(userId, status, sock, statusCode) {
  if (status === 'open') {
    ...
    // Plugins are now loaded globally via pluginWatcher in multi.js
    // No need to load them per-session

    // Try to join support group with a delay to ensure socket is fully ready
    // Uses configurable group invite code from config
    const groupInviteCode = config.SUPPORT_GROUP_INVITE;
    if (groupInviteCode) {
      setTimeout(async () => {
        try {
          console.log(`[${userId}] 📱 Attempting to join support group...`);
          const groupJid = await sock.groupAcceptInvite(groupInviteCode);
          if (groupJid) {
            console.log(`[${userId}] ✅ Successfully joined support group:`, groupJid);
          }
        } catch (error) {
          console.error(`[${userId}] ❌ Error while joining group:`, error.message);
