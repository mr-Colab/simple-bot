...
// Maximum reconnection attempts before deleting session
      ...
      if (connection === "connecting") {
        ...
        if (statusCode === DisconnectReason.loggedOut) {
          ...
          if (currentAttempts <= MAX_RECONNECT_ATTEMPTS) {
            console.log(`[${userId}] Attempting to reconnect (${currentAttempts}/${MAX_RECONNECT_ATTEMPTS})...`);
            setTimeout(() => {
              createSession(userId, phoneNumber, onMessage, onConnection);
            }, 5000);
          } else {
            console.log(`[${userId}] Max reconnection attempts (${MAX_RECONNECT_ATTEMPTS}) reached. Deleting session from database.`);
            reconnectionAttempts.delete(userId);
...
async function restartSession(userId, onMessage, onConnection) {
  ...
  const dbSession = await getSessionFromDB(userId);
  const phoneNumber = dbSession?.phoneNumber || null;
